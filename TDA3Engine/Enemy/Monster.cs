...
namespace TDA3Engine
{
    ...
    public class Monster : GameplayObject
    {
        ...
        public void hit(Bullet bullet, Tower Owner)
        {
            ...
            if (Type == MonsterType.Ice && bullet.Element == ElementType.Fire || Type == MonsterType.Grass && bullet.Element == ElementType.Fire)
            {
                Health -= Convert.ToInt32(Owner.CurrentStatistics.Damage * 1.5);
            } else
                if (Type == MonsterType.Fire && bullet.Element == ElementType.Ice || Type == MonsterType.Grass && bullet.Element == ElementType.Ice)
                {
                    Health -= Convert.ToInt32(Owner.CurrentStatistics.Damage * 0.75);
                }
                else
                {
        ...
        public class MonsterReader : ContentTypeReader<Monster>
        {
            protected override Monster Read(ContentReader input, Monster existingInstance)
            {
                ...
                result.Type = (MonsterType)Enum.Parse(typeof(MonsterType), input.ReadString());
                result.PathIndex = 0;
